...
parser = argparse.ArgumentParser(description="For Cylinder")
parser.add_argument("-m", "--material", type=str, default="Au")
parser.add_argument("-r", "--radius", type=float, default=0.015)
parser.add_argument('-l', "--height", type=float, default=0.06)
parser.add_argument("--wmin", type = float)

args = parser.parse_args()

r = args.radius  # radius of cylinder
h = args.height  # height of cylinder
...
data.to_csv(f'data/cylinder/cylinder_{args.material.lower()}_sca_{r}.csv', index=False)
...
#     plt.title(f'Scattering Cross Section of a {args.material} Cylinder of radius {r} um and length {h} um')
#     plt.tight_layout()
#     plt.savefig(f"plots/cylinder/cylinder_scat_{args.material}_{r}.png")